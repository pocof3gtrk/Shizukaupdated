# if you are repo chor and copy paster then i fucked your mother 🤡

from PurviBots import PurviBots

START = f"""**
❖ ʜᴇʏ ɪ'ᴍ {PurviBots.mention}

:⧽ ᴀɴ ᴀɪ ʙᴀsᴇᴅ ᴄʜᴀᴛ-ʙᴏᴛ.

» ᴄʜᴀᴛ ʟɪᴋᴇ ʜᴜᴍᴀɴ ᴅᴍ + ɢʀᴏᴜᴘ 😘
» ɴᴏ ᴀʙᴜsᴇ, ɴᴏ ᴀᴅs, ᴢᴇʀᴏ ᴅᴏᴡɴᴛɪᴍᴇ.
» ᴘᴏᴡᴇʀғᴜʟʟ + ᴜsᴇғᴜʟʟ & ᴍᴏʀᴇ ғᴜɴᴄᴛɪᴏɴs.

➻ ᴄʟɪᴄᴋ 'ʜᴇʟᴘ' ᴛᴏ sᴇᴇ ᴀʟʟ ᴄᴏᴍᴍᴀɴᴅs.**
"""

HELP_ABOUT =f"""**❖ {PurviBots.mention} ᴀɴ ᴀɪ ʙᴀsᴇᴅ ᴄʜᴀᴛ-ʙᴏᴛ, ᴄʜᴀᴛ ʟɪᴋᴇ ʜᴜᴍᴀɴ ᴅᴍ + ɢʀᴏᴜᴘ 😘
─────────────────────────
● ᴡʀɪᴛᴛєη ɪη » [ᴩʏᴛʜση](https://www.python.org)
● ᴅᴧᴛᴧʙᴧsє » [ϻᴏηɢᴏ-ᴅʙ](https://www.mongodb.com)
● ʜєʟᴘ ᴡɪᴛʜ » [ᴘʏʀσɢʀᴧϻ](https://github.com/pyrogram/pyrogram)
─────────────────────────
➥ ɴᴏ ᴀʙᴜsᴇ, ɴᴏ ᴀᴅs, ᴢᴇʀᴏ ᴅᴏᴡɴᴛɪᴍᴇ.
➥ ᴘʀᴏᴍᴏᴛᴇ ᴍᴇ ᴀᴅᴍɪɴ ᴡɪᴛʜ ʙᴀsɪᴄ ʀɪɢʜᴛs.
➥ ᴧᴅᴅ ϻє ηᴏᴡ ʙᴧʙʏ ɪɴ ʏᴏᴜʀ ɢʀσᴜᴘs.
─────────────────────────
❖ υᴘᴅᴧᴛєs ᴄʜᴧηηєʟ ➥ [ᴘᴜʀᴠɪ-ʙᴏᴛs](https://t.me/PURVI_BOTS)
❖ sυᴘᴘσʀᴛ ᴄʜᴧᴛ ➥ [ᴘᴜʀᴠɪ-ᴜᴘᴅᴀᴛᴇs](https://t.me/PURVI_UPDATES)
─────────────────────────
➤ ʙσᴛ sᴛᴧᴛυs & ϻσʀє ʙσᴛs - [ᴄʟɪᴄᴋ ʜєʀє](https://t.me/Purvibots)
➤ ᴘᴧɪᴅ ᴘʀσϻσᴛɪση :- [ᴄσηᴛᴧᴄᴛ ʜєʀє](https://t.me/TheSigmaCoder)**
"""

HELP_MAIN =f"""**❖ ᴄʜσσsє ᴄᴧᴛєɢσʀʏ ғσʀ ʏσυ ɢєᴛ ʜєʟᴩ.

» ɪꜰ ᴀɴʏ ɪssᴜᴇ ᴄᴏᴍᴇs - [sᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ](t.me/purvi_updates)**
"""


HELP_BASIC =f"""
**<u>⊚ {PurviBots.mention} ʙᴀsɪᴄ ʜᴇʟᴘ :</u>**

• /start - sᴛᴀʀᴛ ᴛʜᴇ ʙᴏᴛ.
• /ping - ᴄʜᴇᴀᴋ ʙᴏᴛ ᴀʟɪᴠᴇ ᴏʀ ᴅᴇᴀᴅ.
• /stats - ᴄʜᴇᴀᴋ ʙᴏᴛ sᴛᴀᴛs.

**✦ 𝐏ᴏᴡᴇʀᴇᴅ вʏ » [ᴀʟᴘʜᴀ-ʙᴀʙʏ](t.me/TheSigmaCoder)**
"""

HELP_CHAT =f"""
**<u>⊚ {PurviBots.mention} ᴄʜᴀᴛ-ʙᴏᴛ ʜᴇʟᴘ :</u>**

• /chatbot - ɪɴʟɪɴᴇ ᴄᴏɴᴛʀᴏʟs ʙᴜᴛᴛᴏɴ ғᴏʀ ᴄʜᴀᴛ.

**✦ 𝐏ᴏᴡᴇʀᴇᴅ вʏ » [ᴀʟᴘʜᴀ-ʙᴀʙʏ](t.me/TheSigmaCoder)**
"""

HELP_INFO =f"""
**<u>⊚ {PurviBots.mention} ɪηꜰσ ʜᴇʟᴘ :</u>**

• /info - ɢєᴛ ɪηғσ ᴧ υsєʀ.
• /id - ɢєᴛ ɢʀσυᴘ/ᴄʜᴀɴɴᴇʟ/ᴜsᴇʀ ɪᴅ.
• /tgm - ᴄʀєᴧᴛє ʟɪηᴋ ᴧηʏ ϻєᴅɪᴧ.
• /tr - ᴛʀᴧηꜱʟᴧᴛє ϻυʟᴛɪᴘʟє ʟᴧηɢυᴧɢєs.

**» ᴄσᴜɴᴛʀʏ ʟᴀɴɢ ᴄσᴅє :- [ᴄʟɪᴄᴋ ʜєʀє](https://telegra.ph/%E1%B4%84%E1%B4%8F%E1%B4%9C%C9%B4%E1%B4%9B%CA%80%CA%8F-%CA%9F%C9%AAs%E1%B4%9B-%E1%B4%A1%C9%AA%E1%B4%9B%CA%9C-%CA%9F%E1%B4%80%C9%B4%C9%A2%E1%B4%9C%E1%B4%80%C9%A2%E1%B4%87-%E1%B4%84%E1%B4%8F%E1%B4%85%E1%B4%87-08-28)**

**✦ 𝐏ᴏᴡᴇʀᴇᴅ вʏ » [ᴀʟᴘʜᴀ-ʙᴀʙʏ](t.me/TheSigmaCoder)**
"""

HELP_TAG =f"""**<u>⊚ {PurviBots.mention} ᴛᴧɢ ʜᴇʟᴘ</u>**

• /rtag - ʀᴧηᴅσϻ ᴛᴧɢ ꜱᴛᴧʀᴛ
• /rstop - ꜱᴛσᴘ ʀᴧηᴅσϻ ᴛᴧɢ

• /utag - ᴜsᴇʀ ᴛᴧɢ sᴛᴀʀᴛ
• /ustop - sᴛᴏᴘ ᴜsᴇᴇ ᴛᴀɢ

• /etag - sᴛᴀʀᴛ ᴇᴍᴏᴊɪ ᴛᴀɢ
• /estop - sᴛᴏᴘ ᴇᴍᴏᴊɪ ᴛᴀɢ

• /atag - ᴀᴅᴍɪɴ ᴛᴀɢ sᴛᴀʀᴛ
• /astop - ꜱᴛσᴘ ᴀᴅᴍɪɴ ᴛᴧɢ

**✦ 𝐏ᴏᴡᴇʀᴇᴅ вʏ » [ᴀʟᴘʜᴀ-ʙᴀʙʏ](t.me/TheSigmaCoder)**
"""

HELP_RANK =f"""
**<u>⊚ {PurviBots.mention} ʀᴀɴᴋɪɴɢ ʜᴇʟᴘ :</u>**

• /today - ᴛᴏᴅᴀʏ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ.
• /weekly - ᴡᴇᴇᴋʟʏ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ.
• /overall - ᴀʟʟ ᴛɪᴍᴇ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ.

**<u>⊚ ʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄʜᴇᴄᴋ ʙʏ ɪɴʟɪɴᴇ ʙᴜᴛᴛᴏɴs.</u>**

• /rank - sʜᴏᴡ ʀᴀɴᴋ ʙᴜᴛᴛᴏɴ.

<blockquote>**» ɪғ ᴀ υsᴇʀ’s ɴᴀᴍᴇ ɪsɴ’ᴛ ᴠɪsɪʙʟᴇ ʙᴜᴛ ᴛʜᴇʏ’ʀᴇ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ, υsᴇ /info user_id ᴛᴏ ᴠɪᴇᴡ ᴛʜᴇɪʀ ᴅᴇᴛᴀɪʟs.**</blockquote>

**✦ 𝐏ᴏᴡᴇʀᴇᴅ вʏ » [ᴀʟᴘʜᴀ-ʙᴀʙʏ](t.me/TheSigmaCoder)**
"""

HELP_WEL =f"""
**<u>⊚ {PurviBots.mention} ᴡᴇʟᴄᴏᴍᴇ ʜᴇʟᴘ :</u>** 

• /welcome - ɪɴʟɪɴᴇ ᴄᴏɴᴛʀᴏʟs ʙᴜᴛᴛᴏɴ ғᴏʀ ᴡᴇʟᴄᴏᴍᴇ.

**✦ 𝐏ᴏᴡᴇʀᴇᴅ вʏ » [ᴀʟᴘʜᴀ-ʙᴀʙʏ](t.me/TheSigmaCoder)**
"""
